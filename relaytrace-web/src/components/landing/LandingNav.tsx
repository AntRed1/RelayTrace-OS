...
/** Smooth-scroll to a section with a fixed-navbar offset */
...
export function LandingNav({ onRequestAccess }: Props) {
  ...
  return (
    <header
      ...
      style={{
        background: scrolled ? "rgba(255,255,255,0.95)" : "rgba(15,23,42,0.0)",
    ...
    >
        ...
        <nav className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map(({ label, id }) => (
            <a
              ...
              className="text-sm font-medium transition-all duration-200 hover:text-blue-400 hover:scale-105"
      ...
      >
        <div className="bg-white border-t border-slate-100 px-6 py-4 space-y-3">
          {NAV_LINKS.map(({ label, id }) => (
            <a
              key={id}
              href={`#${id}`}
              onClick={(e) => handleNavClick(e, id)}
              className="block text-sm font-medium text-slate-600 py-1.5 transition-colors duration-200 hover:text-blue-600"
            >
              {label}
            </a>
          ))}
          <div className="pt-2 flex flex-col gap-2">
            <Link
              href={ROUTES.AUTH.LOGIN}
              className="w-full text-center py-2.5 rounded-xl text-sm font-semibold text-blue-600 border border-blue-200 transition-all duration-200 hover:bg-blue-50"
            >
              Sign In
            </Link>
            <button
              onClick={() => { onRequestAccess(); setMobileOpen(false); }}
              className="w-full py-2.5 rounded-xl text-sm font-bold text-white transition-all duration-200 hover:opacity-90 active:scale-[0.98]"
